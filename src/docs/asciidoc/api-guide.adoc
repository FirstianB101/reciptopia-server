...
[[resources_accountProfileImg_retrieve]]
=== AccountProfileImg 조회

`GET /account/profileImgs/{id}` 요청으로 하나의 AccountProfileImg를 조회(리소스 다운로드)합니다. +
유효하지 않은 `id` 가 입력되면 `404 Not Found` 에러를 반환합니다.

operation::accountProfileImg-retrieve-example[snippets='response-fields,curl-request,http-response']

[[resources_accountProfileImg_list]]
=== AccountProfileImg 목록 조회

`GET /account/profileImgs` 요청으로 다수의 AccountProfileImg들을 조회(엔티티 정보 조회)합니다.
<<overview_paging_and_sorting, Paging 혹은 Sorting>> 요청 패러미터를 추가하면 원하는 방식으로 목록을 조회할 수 있습니다.
...
`POST /account/profileImgs` 요청으로 AccountProfileImg를 생성합니다. +
...
operation::accountProfileImg-create-example[snippets='request-fields,curl-request,http-response']

[[resources_accountProfileImg_delete]]
=== AccountProfileImg 삭제

`DELETE /account/profileImgs/{id}` 요청으로 AccountProfileImg를 삭제합니다. +
